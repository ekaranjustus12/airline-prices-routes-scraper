...
.search-panel { background:#ffffff; border:1px solid #e5e7eb; border-radius:12px; padding:1.1rem 1.25rem; margin-bottom:1.25rem; }
...
.flight-card { background:#ffffff; border:1px solid #e5e7eb; border-radius:12px; padding:1rem 1.25rem; margin-bottom:10px; }
...
.book-btn  { font-size:13px; padding:7px 16px; border-radius:8px; border:1px solid #e5e7eb; background:#f9fafb; color:#374151; cursor:pointer; font-family:inherit; text-decoration:none; display:inline-block; }
.best-card .book-btn { background:#E1F5EE; border-color:#9FE1CB; color:#0F6E56; }

.refresh-row { display:flex; align-items:center; justify-content:space-between; margin-top:14px; padding-top:12px; border-top:1px solid #f3f4f6; }
...
def load_data() -> pd.DataFrame:
    ...
    # FIX: use df[col] not df.get(col) — DataFrame.get() returns a scalar
    # default for missing columns, which silently breaks pd.to_numeric.
    for col, default in [("price_kes", 0), ("price_usd", 0), ("stops", 0)]:
    ...
    # FIX: times are stored as bare "HH:MM" strings — pd.to_datetime coerces
    # them to NaT. Just clean as strings instead.
...
def render_card(row, is_best: bool, route_info: dict):
    ...
    initials  = "".join(w[0] for w in str(airline).split()[:2]).upper()
    ...
    dur_raw = row.get("duration_mins", "")
    try:
        dur_mins = int(float(dur_raw))
        dur_label = f"{dur_mins // 60} h {dur_mins % 60} min" if dur_mins >= 60 else f"{dur_mins} min"
...
if not df.empty:
    filtered = df[
        ...
        (df["flight_date"] == date_str)   # FIX: was >= (showed all future dates)
...
if filtered.empty:
    if df.empty:
        ...
        <div class="empty-state">
          <h3>No data yet</h3>
          <p>Run the scraper to fetch live prices.</p>
