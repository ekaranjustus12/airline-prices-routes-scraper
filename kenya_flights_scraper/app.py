...
[data-testid="stMetricValue"] {
    ...
    font-weight: 800 !important; color: #00d4aa !important;
}
...
[data-testid="stSidebar"] p { color: #e8edf5 !important; }
...
.fc-airline { font-size:15px; font-weight:600; color:#e8edf5; }
.fc-route   { font-size:12px; color:#7a8ba0; margin-top:2px; }
.fc-time-block { text-align:center; }
.fc-time    { font-family:'Syne',sans-serif; font-size:22px; font-weight:700; color:#e8edf5; line-height:1; }
.fc-tlabel  { font-size:10px; color:#7a8ba0; margin-top:2px; }
.fc-dur     { font-size:11px; color:#7a8ba0; text-align:center; padding:0 8px; }
...
def load_data() -> pd.DataFrame:
    ...
    # FIX: use df[col] not df.get(col, default) — DataFrame.get() returns
    # a scalar for missing columns which breaks pd.to_numeric downstream.
    for col, default in [("price_kes", 0), ("price_usd", 0.0),
                         ("stops", 0), ("duration_mins", 0)]:
    ...
    # FIX: times are stored as bare "HH:MM" strings — don't use pd.to_datetime
    # which coerces them to NaT.  Just clean as strings.
...
def render_flight_card(row: dict, is_cheapest: bool = False) -> str:
    ...
    <div class="flight-card {'cheapest' if is_cheapest else ''}">
      <div>
        {badge}
        <div class="fc-airline">{row.get('airline','Unknown')}</div>
        <div class="fc-route">{row.get('route','')}</div>
      </div>
      <div style="display:flex;align-items:center;gap:10px">
        <div class="fc-time-block">
          <div class="fc-time">{dep}</div>
          <div class="fc-tlabel">Dep</div>
        </div>
        <div class="fc-dur">──{dur}──</div>
        <div class="fc-time-block">
          <div class="fc-time">{arr}</div>
          <div class="fc-tlabel">Arr</div>
        </div>
      </div>
      <div style="text-align:center">
        {stops_badge(row.get('stops', 0))}
        <div style="font-size:11px;color:#7a8ba0;margin-top:4px">{row.get('flight_date','')}</div>
      </div>
      <div>
...
def run_scraper_thread(route_label=None, flight_date=None):
    ...
    try:
        ...
        # Surface stdout for debugging
        st.session_state.scrape_log = (result.stdout or "")[-2000:]

        if result.returncode == 0:
            st.session_state.scrape_status  = "success"
            st.session_state.scrape_message = "Scrape completed! Data updated."
        else:
            # Show the REAL error — previously stderr was never exposed
            stderr_tail = (result.stderr or "No stderr captured")[-600:]
            st.session_state.scrape_status  = "error"
            st.session_state.scrape_message = stderr_tail
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("## ✈ Kenya Flights")
    st.markdown("---")

    st.markdown("### 🔄 Live Scrape")
    scrape_route = st.selectbox(
        "Route to scrape",
        ["All routes"] + list(ROUTES.keys()),
        key="scrape_route_sel",
    )
    ...
    if scrape_btn and not st.session_state.scrape_running:
        label = (
            None if scrape_route == "All routes"
            else ROUTES[scrape_route]["label"]
        )
        threading.Thread(
            target=run_scraper_thread,
            args=(label, scrape_date),
            daemon=True,
        ).start()
        st.rerun()

    # Status display
    status = st.session_state.scrape_status
    msg    = st.session_state.scrape_message
    if status == "running":
        st.markdown(f'<div class="status-running">⚡ {msg}</div>', unsafe_allow_html=True)
    elif status == "success":
        st.markdown(f'<div class="status-success">✓ {msg}</div>', unsafe_allow_html=True)
        if st.session_state.last_run:
            st.caption(f"Last run: {st.session_state.last_run}")
    elif status == "error":
        st.markdown(f'<div class="status-error">✗ Scraper error</div>', unsafe_allow_html=True)
        st.code(msg, language="bash")   # monospace — easier to read stack traces

    # Scraper stdout log (collapsed)
    ...
    f_route   = st.selectbox("Route",   route_options)
    f_date    = st.selectbox("Date",    date_options)
    f_airline = st.selectbox("Airline", airline_options)
    f_stops   = st.radio("Stops", ["Any", "Direct only", "1 stop"], horizontal=True)
    ...
    if auto_refresh:
        st.caption("Page will auto-refresh while toggled on.")
...
with col_title:
    st.markdown("# ✈ Kenya Flights")
    st.caption("Real-time scraped prices from eSky Kenya")
with col_ts:
    if st.session_state.last_run:
        st.markdown(
            f"<div style='text-align:right;color:#7a8ba0;font-size:12px;padding-top:20px'>"
            f"Last updated<br><strong style='color:#e8edf5'>{st.session_state.last_run}</strong></div>",
            unsafe_allow_html=True,
        )

# ── Best-price metric cards ────────────────────────────────────────────────────
cheapest_map = {}
if not df.empty:
    ...
    for i, (route_name, meta) in enumerate(ROUTES.items()):
        label  = meta["label"]
        subset = df[df["route"] == label]
        with stat_cols[i]:
            if not subset.empty:
                best = subset.loc[subset["price_kes"].idxmin()]
                cheapest_map[label] = int(best["price_kes"])
                st.metric(
                    label=route_name,
                    value=fmt_kes(best["price_kes"]),
                    delta=f"{best['airline']} · {best['flight_date']}",
                    delta_color="off",
                )
            else:
                st.metric(label=route_name, value="No data", delta="Run scraper →")
    st.markdown("---")
...
if not filtered.empty:
    ...
    # FIX: was >= date_str (showed every future date). Use == to match selection only.
    if f_date    != "All dates":    filtered = filtered[filtered["flight_date"] == f_date]
    if f_airline != "All airlines": filtered = filtered[
        filtered["airline"].str.contains(f_airline, case=False, na=False)
    ]
    if f_stops == "Direct only":    filtered = filtered[filtered["stops"] == 0]
    elif f_stops == "1 stop":       filtered = filtered[filtered["stops"] == 1]

    filtered = filtered.sort_values(
        sort_map[f_sort], ascending=(f_order == "Cheapest first")
    )
...
with res_col:
    ...
    st.markdown(
        f'<div class="section-head">Showing {total} flight{"s" if total != 1 else ""}</div>',
        unsafe_allow_html=True,
    )
with refresh_col:
...
if filtered.empty:
    if df.empty:
        ...
        <div class="empty-state">
          ...
          <strong style="font-size:18px;color:#e8edf5;font-family:Syne,sans-serif">No flight data yet</strong>
          <p style="margin-top:8px">Click <strong>▶ Run Scraper</strong> in the sidebar to fetch live prices.</p>
        ...
        <div class="empty-state">
          <div style="font-size:48px;margin-bottom:12px">🔍</div>
          <strong style="font-size:18px;color:#e8edf5;font-family:Syne,sans-serif">No flights match your filters</strong>
    ...
    for _, row in filtered.iterrows():
        is_cheapest = (
            row["route"] in cheapest_map
            and int(row["price_kes"]) <= cheapest_map[row["route"]]
        )
